...
np.random.seed(1337)
from .utils.augment import ImgAugment, make_jitter_on_image
    ...
    class BatchGenerator(Sequence):
        ...
        def __getitem__(self, idx):
            ...
            for i in range(self._batch_size):
                ...
                if self.annotations.is_img_in_memory:
                    img = self.annotations.img(self._batch_size * idx + i)
                    img, boxes = self._img_aug.make_jitter(img, boxes)
                else:
...
class _YoloBox(object):
    ...
    def trans(self, boxes):
        ...
        h_r = (self._grid_size[0] / self._input_size[0])
        w_r = (self._grid_size[1] / self._input_size[1])
        norm_boxes = centroid_boxes * (w_r, h_r, w_r, h_r)
        return norm_boxes
...
class _NetoutGen(object):
    ...
    def _generate_y(self, best_anchor, obj_indx, box):
        ...
        try:
            ...
            print(
                "y.shape:{}, grid_x:{}, grid_y:{}, best_anchor:{}, box:{}".format(y.shape, grid_x, grid_y, best_anchor,
                                                                                  box))
